    ...
    def setup(self, stage=None):
        ...
        if not os.path.exists(self.train_path):
            raise FileNotFoundError(
                f"Data not found at {self.train_path}. Use math_classifier/commands.py mode=download!"
            )
        ...
        print(
            f"Loaded {len(full_train_df)} raw training samples and {len(test_df)} test samples."
        )
        ...
        train_df, val_df = train_test_split(
            ...
            stratify=full_train_df[
                "type"
            ],  # Good practice: keep label distribution similar
        )
        ...
        self.vectorizer = TfidfVectorizer(
            max_features=self.model_cfg.max_features, stop_words="english"
        )
